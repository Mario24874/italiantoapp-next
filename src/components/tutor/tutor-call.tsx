...
import { Mic, MicOff, PhoneOff, Settings, Volume2, VolumeX, ArrowLeft, AlertCircle, Check } from 'lucide-react'
import { cn } from '@/lib/utils'
import { TUTOR_VOICES, resolveWebSpeechVoice } from './tutor-voices'

// ── Types ────────────────────────────────────────────────────────────────────
type CallStatus = 'idle' | 'listening' | 'thinking' | 'speaking'
type AvatarSlug = 'marco' | 'giovanni' | 'giulia' | 'francesca'
...
const VOICE_MAP: Record<'male' | 'female', { id: string; label: string }[]> = {
  male: [
...
// ── Main component ────────────────────────────────────────────────────────────
export function TutorCall({ tutor }: { tutor: { slug: string; name: string } }) {
  ...
  const [prefs, setPrefs]               = useState<TutorPrefs>({
    ...DEFAULT_PREFS,
    avatarSlug: (tutor.slug as AvatarSlug) ?? 'marco',
    customName: tutor.name,
    voiceId: TUTOR_VOICES.find(t => t.slug === tutor.slug)?.elevenlabsVoiceId ?? DEFAULT_PREFS.voiceId,
  })
  const [nameInput, setNameInput]       = useState(tutor.name)
  ...
  // Derived values from prefs
  const avatarData    = TUTOR_VOICES.find(t => t.slug === prefs.avatarSlug) ?? TUTOR_VOICES[0]
  const effectiveName = prefs.customName || tutor.name
  const effectiveVoice = prefs.voiceId
  const voiceOptions  = VOICE_MAP[avatarData.gender]
  ...
  useEffect(() => {
    try {
      ...
      if (s) {
        const loaded = { ...DEFAULT_PREFS, avatarSlug: tutor.slug as AvatarSlug, customName: tutor.name, ...JSON.parse(s) }
        setPrefs(loaded)
        setNameInput(loaded.customName)
      }
    } catch {}
  }, [tutor.slug, tutor.name])
  ...
  const startMicAnalysis = useCallback(async () => {
    ...
    } catch { setError('Permesso microfono negato.') }
  }, [])
  ...
  const speak = useCallback(async (text: string) => {
    ...
    utter.lang = 'it-IT'; utter.rate = 0.9; utter.pitch = avatarData.pitch
    const setVoice = () => { const v = resolveWebSpeechVoice(avatarData.gender); if (v) utter.voice = v }
  ...
  }, [ttsEnabled, effectiveVoice, avatarData, stopAudio])
  ...
  }, [effectiveName, tutor.slug, prefs, speak])
  ...
  // ── Greeting ───────────────────────────────────────────────────────────────
  useEffect(() => {
    const greeting = `Ciao! Sono ${effectiveName}. Benvenuto! Parli italiano, español o English?`
    ...
    return (
      <div className="flex flex-col h-full bg-[#f5f7f5] dark:bg-[#0d1a0d]">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-6">
          ...
          {/* ── Avatar selector ── */}
          <div className="space-y-2">
            <p className="text-xs font-semibold text-italianto-700 dark:text-italianto-500 uppercase tracking-wide">Avatar del tutor</p>
            <div className="grid grid-cols-4 gap-2">
              {TUTOR_VOICES.map(av => (
                <button
                  key={av.slug}
                  onClick={() => savePrefs({ ...prefs, avatarSlug: av.slug as AvatarSlug, voiceId: av.elevenlabsVoiceId ?? prefs.voiceId })}
                  className={cn(
                    'flex flex-col items-center gap-1.5 p-1.5 rounded-xl border transition-all',
                    prefs.avatarSlug === av.slug
                ...
                >
                  <div className="relative size-14 rounded-full overflow-hidden">
                    ...
                    <img src={av.avatar} alt={av.name} className="size-full object-cover" />
                    {prefs.avatarSlug === av.slug && (
          ...
          {/* ── Custom name ── */}
          <div className="space-y-2">
            <p className="text-xs font-semibold text-italianto-700 dark:text-italianto-500 uppercase tracking-wide">Nombre del tutor</p>
            <div className="flex gap-2">
              <input
                type="text"
                value={nameInput}
                onChange={e => setNameInput(e.target.value)}
                placeholder={avatarData.name}
                maxLength={24}
              ...
              />
              <button
                onClick={() => savePrefs({ ...prefs, customName: nameInput.trim() || avatarData.name })}
                className="px-3 py-2 bg-italianto-700 hover:bg-italianto-800 text-white text-xs font-semibold rounded-xl transition-colors"
              >
                OK
              </button>
            </div>
          ...
          {/* ── Voice selector ── */}
          <div className="space-y-2">
            <p className="text-xs font-semibold text-italianto-700 dark:text-italianto-500 uppercase tracking-wide">Voz (ElevenLabs)</p>
            <div className="flex gap-2">
              {voiceOptions.map(v => (
                <button
                  key={v.id}
                  ...
                  className={cn(
                    ...
                    prefs.voiceId === v.id
                      ? 'bg-italianto-700 border-italianto-600 text-white dark:bg-italianto-800'
                      : 'bg-white dark:bg-[#132213] border-[#d4e4d4] dark:border-[#1e3a1e] text-gray-500 dark:text-[#4a7a4a] hover:border-italianto-400',
                  )}
                >
                  {v.label}
            ...
            <p className="text-[10px] text-gray-400 dark:text-[#2a4a2a]">
              Requiere ELEVENLABS_API_KEY. Sin clave usa Web Speech API.
            </p>
          </div>

          <div className="border-t border-[#d4e4d4] dark:border-[#1e3a1e] pt-5 space-y-5">
            <p className="text-xs font-semibold text-italianto-700 dark:text-italianto-500 uppercase tracking-wide">Preferenze sessione</p>
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f7f5] dark:bg-[#0d1a0d]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#d4e4d4] dark:border-[#1e3a1e] shrink-0 bg-white/80 dark:bg-[#0d1a0d]/80 backdrop-blur">
        <button onClick={() => { stopAudio(); router.push('/tutor') }}
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => { stopAudio(); setTtsEnabled(v => !v) }}
      ...
      <div className="flex flex-col items-center pt-6 pb-4 shrink-0">
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={avatarData.avatar} alt={effectiveName} className="size-full object-cover"
        ...
        <p className="text-sm text-gray-400 dark:text-[#4a7a4a]">{avatarData.description}</p>
      ...
      <div className="flex justify-center items-center gap-6 px-8 pb-6 pt-3 shrink-0">
        {isActive && (
          <button onClick={() => { stopAudio(); stopListening(); setStatus('idle') }}
        ...
        <motion.button whileTap={{ scale: 0.93 }}
          ...
          className={cn(
            ...
            isListening ? 'bg-red-500 text-white ring-4 ring-red-300/50'
              : isThinking || isSpeaking ? 'bg-gray-200 dark:bg-[#1e3a1e] text-gray-400 cursor-not-allowed'
